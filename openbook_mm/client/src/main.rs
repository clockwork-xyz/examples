...
fn main() -> ClientResult<()> {
    ...
    let sol_usdc_market_keys = MarketKeys {
        ...
        coin_mint: Pubkey::from_str("So11111111111111111111111111111111111111112").unwrap(),
    ...
    dca_create(&client, &sol_usdc_market_keys, "BONK_USDC_DCAbb".into(),"BONK_USDC_DCAaa".into())?;
...
fn dca_create(
    ...
    let dca_thread_pubkey2 = Thread::pubkey(client.payer_pubkey(), dca_thread_id2.clone());
    ...
    let thread_create_swap_ix_ask = thread_create(
        client.payer_pubkey(),
        dca_thread_id,
        Instruction {
            program_id: openbook_dca::ID,
            ...
            data: openbook_dca::instruction::SwapAsk {}.data(),
        }
        .into(),
        client.payer_pubkey(),
        dca_thread_pubkey,
        Trigger::Account {
            address: market_keys.asks,
            offset: 8,
            size: 65548,
        },
    );
    ...
    let thread_create_swap_ix = thread_create(
        ...
        Instruction {
            ...
            accounts: vec![
                ...
                AccountMeta::new(dca_coin_vault_pubkey, false),
                AccountMeta::new(dca_pc_vault_pubkey, false),
                AccountMeta::new_readonly(dca_thread_pubkey2, true),
                AccountMeta::new_readonly(sysvar::rent::ID, false),
                AccountMeta::new_readonly(system_program::ID, false),
                AccountMeta::new_readonly(token::ID, false),
                // REMAINING ACCOUNTS
    ...
    let fund_swap_thread_ix = transfer(&client.payer_pubkey(), &dca_thread_pubkey2, 100000000);

    {
        ...
        print_explorer_link(dca_thread_pubkey2, "dca thread2 📂".into())?;
    ...
    sign_send_and_confirm_tx(
        &client,
        [
            thread_create_swap_ix_ask, // on schedule: transfer & swap; transfer & swap; ...
        ]
        .to_vec(),
        Some(vec![client.payer()]),
        "swap thread create".to_string(),
